...
\newcommand{\exploratorypath}{/Users/jeffreyohl/Dropbox/LLM_PassThrough/output/exploratory}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{pre\_mean\_early}: mean log(unique users) Mar--Jun 2023
        \item \texttt{pre\_mean\_late}: mean log(unique users) Jul--Sep 2023
...
\textbf{Outcome:} $\log(\text{unique cardids with \$15--\$25 transaction})$
per ZIP3-month.
...
(Figure~\ref{fig:taxdetect}). This may indicate OpenAI began collecting
New York sales tax during the sample period. If so, Manhattan experienced
a (smaller) tax treatment of its own, which would justify excluding it
from the donor pool. We have not yet made this exclusion.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{\exploratorypath/zip3_price_change_funnel.png}
...
\section{Placebo Price Plots}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{\figpath/median_price_077.png}
\caption{Median transaction price for ZIP3 077 (Long Branch / Red Bank, NJ).
No discrete price jump is visible, despite this area having the highest
RMSPE ratio ({\color{red}6.90}) among placebo units tested so far.}
\label{fig:077_price}
\end{figure}
